...
const fetchJoin = async () => {
  const url = 'http://4.230.153.181:8080/customers'; // API 엔드포인트 추가
  try {
...
const addLocation = async (moveId) => {
  const url = 'http://4.230.153.181:8080/locations'; // API 엔드포인트
...
const url = 'http://4.230.153.181:8080/moves/startmove'; // API 엔드포인트
...
const mm = String(currentDate.getMinutes()).padStart(2, '0'); // 분
const ss = String(currentDate.getSeconds()).padStart(2, '0'); // 초
moveId.value = "move-" + yyyy + MM + dd + HH + mm + ss;
try {
...
intervalId.value = setInterval(async () => {
  try {
    // 현재 이동 상태
    const response = await axios.get(`http://4.230.153.181:8080/moves/${moveId.value}`);
    ...
    if (response.data.status === 'start' || response.data.status === 'moving') {
      ...
      await axios.post('http://4.230.153.181:8080/moves/updatemove', {
          moveId: moveId.value,
      });
      ...
      // 이동 완료 시 end API 호출
      await axios.post('http://4.230.153.181:8080/moves/endmove', {
        moveId: moveId.value,
        // 필요 시 추가 데이터
      });
